...
int main()
{
	const char *card0[max_n] = { // 54张牌存于card0中 
	...
	const char *card1[max_n]; // 洗牌后存于card1中 
	
	int k; // 洗牌次数 
	scanf("%d", &k); 
	
	int order[max_n]; // 洗牌用的数字 
	for (int i = 1; i <= 54; i++) {
		scanf("%d", &order[i]);
	}
	
	for (int i = 1; i <= k; i++) { // 洗牌k次，在card0和card1间来回倒牌 
		for (int j = 1; j <= 54; j++) { 
			if (i % 2 == 1) // 第1次（奇数次），从card0倒入card1 
				card1[order[j]] = card0[j];
			else // 第2次（偶数次），从card1倒回card0 
				card0[order[j]] = card1[j]; 
		}
	}
	
	for (int i = 1; i <= 54; i++) {
		if (k % 2 == 1) { // 洗奇数次牌，输出card1 
			printf("%s", card1[i]);
		} else { // 洗偶数次牌，输出card0
			printf("%s", card0[i]);
		}
		if (i < 54) printf(" "); // 末尾无空格 
	}
	return 0;
}
